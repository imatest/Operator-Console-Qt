...
#ifndef PASSFAIL_SETTINGS_H
#define PASSFAIL_SETTINGS_H
...
{
    std::string name;				//!< the pass/fail variable (or key) name as given in the Imatest documentation
    std::string description;		//!< The description of this variable to be displayed.
...
{
    std::string group_name;			//!< The overall name for the pair
    std::string min_name;			//!< The name displayed for the min entry
                                    /**< Note that an MFC property grid will not display a description for the group*/
    std::string max_name;			//!< The name displayed for the max entry
                                    /**< Note that an MFC property grid will not display a description for the group*/
    std::string min_description;	//!< The description displayed for the min variable
    std::string max_description;	//!< The description displayed for the max variable
...
{
    std::string group_name;						//!< The name to be displayed for the collection
    std::string group_description;				//!< The description for the collection
    ...
    std::vector<std::string> sub_names;			//!< Names to be displayed for each element of the variable
    std::vector<std::string> sub_descriptions;	//!< Descriptions to be displayed for each element
											/**< If you want all values to have the same description, you can leave sub_descriptions empty and place the description in group_description*/
...
typedef struct SFRplusPassFail
{
    ...
    std::string name;			//!< The name to be displayed for this assembly of variables.
...
///
/// A struct that can be used to catch other (that is, not Blemish, OIS, or SFRplus) pass/fail categories into a vector of entry<std::string>'s
///
typedef struct OtherPassFail{
    std::vector<entry<std::string> > ent_vec;
...
{
public:
    PassFailSettings(void);				//!< The constructor, which contains all the initializations for every pass/fail variable
    ~PassFailSettings(void);
	SFRplusPassFail sfrplus;
	Blemish blemish;
	OISPassFail ois;
	OtherPassFail other;
	bool b_isReadOnly;						//!< for checking if the Pass/Fail file is read-only
    std::string m_ini_file;						//!< fully-qualified filename for the Imatest ini file
    std::string m_pass_fail_file;				//!< fully-qualified filename for the Pass/Fail file
};
