...
bool PassFail::Read(PassFailSettings &settings)
{
    ...
    QFileInfo info(settings.m_pass_fail_file);
    if (!info.isWritable())
    {
    ...
    mwArray sections;
    mwArray subsections;
    mwArray passFailFile(settings.m_pass_fail_file.toStdString().c_str());
    ...
    */
    ...
    if (settings.blemish.b_enable)
    {
        ...
        mwArray section_Blemish(settings.blemish.name.toStdString().c_str());

        varargin = mwArray(1,3,mxCELL_CLASS);

        std::vector<std::string> data_types;
        std::vector<QString> blem_keys;
        ...
        for (std::size_t idx = 0; idx < settings.blemish.numEntries; ++idx)
        {
            try
            {
                mwArray key(blem_keys[idx].toStdString().c_str());
    ...
    if ( settings.sfrplus.b_enable)
    {
        ...
        mwArray section_sfr(settings.sfrplus.name.toStdString().c_str());

        varargin = mwArray(1,3,mxCELL_CLASS);

        std::vector<std::string> data_types;
        std::vector<QString> sfr_keys;
        ...
        {
            mwArray key(sfr_keys[idx].toStdString().c_str());
            mwArray value(data_types[idx].c_str());
            mwArray default_ret;
    ...
    if (settings.ois.b_enable)
    {
        ...
        varargin = mwArray(1,3,mxCELL_CLASS);
        mwArray section_ois(settings.ois.name.toStdString().c_str());

        std::vector<std::string> data_types;
        std::vector<QString> ois_keys;
        ...
        {
            mwArray key(ois_keys[idx].toStdString().c_str());
            mwArray value(data_types[idx].c_str());
            mwArray default_ret;
...
bool PassFail::Write(void)
{
    bool result = false;

    mwArray varargin = mwArray(1,4,mxCELL_CLASS);

    mwArray inifilename(settings.m_ini_file.toStdString().c_str());
    mwArray passfailfilename(settings.m_pass_fail_file.toStdString().c_str());
    mwArray mode("write");
    mwArray style("plain");
    mwArray subsection_blank("");
    mwArray value_int("i"), value_string(""), value_double("d");
    ...
    if (settings.blemish.b_enable)
    {
        mwArray section(settings.blemish.name.toStdString().c_str());
        std::vector<std::string> keys;
        std::vector<std::string> vals;

        addKeysAndValues(settings.blemish.Blemish_maximum_count.group_name.toStdString().c_str(), writeVecEntValueString(settings.blemish.Blemish_maximum_count), keys, vals, settings.blemish.Blemish_maximum_count.b_isUsed);				// 1
        addKeysAndValues(settings.blemish.Blemish_size_pixels.group_name.toStdString().c_str(),writeVecEntValueString(settings.blemish.Blemish_size_pixels),keys,vals,settings.blemish.Blemish_size_pixels.b_isUsed);						// 2
        addKeysAndValues(settings.blemish.Dead_pixels_max.name.toStdString().c_str(),writeEntValueString(settings.blemish.Dead_pixels_max), keys, vals, settings.blemish.Dead_pixels_max.b_isUsed);											// 3
        addKeysAndValues(settings.blemish.Dead_pixel_clusters_max.name.toStdString().c_str(), writeEntValueString(settings.blemish.Dead_pixel_clusters_max), keys, vals, settings.blemish.Dead_pixel_clusters_max.b_isUsed);				// 4
        addKeysAndValues(settings.blemish.Defective_pixels_max_count.name.toStdString().c_str(), writeEntValueString(settings.blemish.Defective_pixels_max_count), keys, vals, settings.blemish.Defective_pixels_max_count.b_isUsed);		// 5
        addKeysAndValues(settings.blemish.Hot_pixel_clusters_max.name.toStdString().c_str(), writeEntValueString(settings.blemish.Hot_pixel_clusters_max), keys, vals, settings.blemish.Hot_pixel_clusters_max.b_isUsed);					// 6
        addKeysAndValues(settings.blemish.Hot_pixels_max.name.toStdString().c_str(), writeEntValueString(settings.blemish.Hot_pixels_max), keys, vals, settings.blemish.Hot_pixels_max.b_isUsed);											// 7
        addKeysAndValues(settings.blemish.Optical_center_offset_max.name.toStdString().c_str(), writeEntValueString(settings.blemish.Optical_center_offset_max), keys, vals, settings.blemish.Optical_center_offset_max.b_isUsed);			// 8
        addKeysAndValues(settings.blemish.Optical_center_offset_X_max.name.toStdString().c_str(), writeEntValueString(settings.blemish.Optical_center_offset_X_max), keys, vals, settings.blemish.Optical_center_offset_X_max.b_isUsed);	// 9
        addKeysAndValues(settings.blemish.Optical_center_offset_Y_max.name.toStdString().c_str(), writeEntValueString(settings.blemish.Optical_center_offset_Y_max), keys, vals, settings.blemish.Optical_center_offset_Y_max.b_isUsed);	//10
        addKeysAndValues(settings.blemish.Relative_illumination_corner_diff_pct_max.name.toStdString().c_str(), writeEntValueString(settings.blemish.Relative_illumination_corner_diff_pct_max), keys, vals, settings.blemish.Relative_illumination_corner_diff_pct_max.b_isUsed);		//11
        addKeysAndValues(settings.blemish.Relative_illumination_worst_corner_pct_min.name.toStdString().c_str(), writeEntValueString(settings.blemish.Relative_illumination_worst_corner_pct_min), keys, vals, settings.blemish.Relative_illumination_worst_corner_pct_min.b_isUsed);	//12
        addKeysAndValues(settings.blemish.Uniformity_BoverG_corners_pct_max.name.toStdString().c_str(), writeEntValueString(settings.blemish.Uniformity_BoverG_corners_pct_max), keys, vals, settings.blemish.Uniformity_BoverG_corners_pct_max.b_isUsed);								//13
        addKeysAndValues(settings.blemish.Uniformity_RoverG_corners_pct_max.name.toStdString().c_str(), writeEntValueString(settings.blemish.Uniformity_RoverG_corners_pct_max), keys, vals, settings.blemish.Uniformity_RoverG_corners_pct_max.b_isUsed);								//14
    ...
    if (settings.ois.b_enable)
    {
        mwArray section(settings.ois.name.toStdString().c_str());
        std::vector<std::string> keys;
        std::vector<std::string> vals;

        addKeysAndValues(settings.ois.L_MTF50_delta2_gain_summary_all_dB_min.name.toStdString().c_str(), writeEntValueString(settings.ois.L_MTF50_delta2_gain_summary_all_dB_min), keys, vals, settings.ois.L_MTF50_delta2_gain_summary_all_dB_min.b_isUsed);// 1
        addKeysAndValues(settings.ois.R_improve_ALL_dB_min.name.toStdString().c_str(), writeEntValueString(settings.ois.R_improve_ALL_dB_min), keys, vals, settings.ois.R_improve_ALL_dB_min.b_isUsed);	// 2
        addKeysAndValues(settings.ois.R_improve_H_dB_min.name.toStdString().c_str(), writeEntValueString(settings.ois.R_improve_H_dB_min), keys, vals, settings.ois.R_improve_H_dB_min.b_isUsed);		// 3
        addKeysAndValues(settings.ois.R_improve_V_dB_min.name.toStdString().c_str(), writeEntValueString(settings.ois.R_improve_V_dB_min), keys, vals, settings.ois.R_improve_V_dB_min.b_isUsed);		// 4
    ...
    if (settings.other.b_enable)
    {
        ...
        mwArray section(settings.other.name.toStdString().c_str());
        // add the contents of ent_vec to writeKeys
        for (std::size_t idx=0; idx < settings.other.ent_vec.size(); ++idx)
        {
            mwArray key(settings.other.ent_vec[idx].name.toStdString().c_str());
    ...
    if ( settings.sfrplus.b_enable)
    {
        mwArray section(settings.sfrplus.name.toStdString().c_str());
        std::vector<std::string> keys;
        std::vector<std::string> vals;

        addKeysAndValues(settings.sfrplus.All_Edge_IDs_detected.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.All_Edge_IDs_detected), keys, vals, settings.sfrplus.All_Edge_IDs_detected.b_isUsed);												// 1
        addKeysAndValues(settings.sfrplus.Bayer_decode.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Bayer_decode), keys, vals, settings.sfrplus.Bayer_decode.b_isUsed);																			// 2
        addKeysAndValues(settings.sfrplus.Chart_mean_pixel_level_bounds.group_name.toStdString().c_str(), writeMinMaxEntValueString(settings.sfrplus.Chart_mean_pixel_level_bounds), keys, vals, settings.sfrplus.Chart_mean_pixel_level_bounds.b_isUsed);			// 3
        addKeysAndValues(settings.sfrplus.Chart_radial_pixel_shift_max.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Chart_radial_pixel_shift_max), keys, vals, settings.sfrplus.Chart_radial_pixel_shift_max.b_isUsed);							// 4
        addKeysAndValues(settings.sfrplus.Color_expected_detected.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Color_expected_detected), keys, vals, settings.sfrplus.Color_expected_detected.b_isUsed);										// 5
        addKeysAndValues(settings.sfrplus.DeltaE_00_mean_max.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.DeltaE_00_mean_max), keys, vals, settings.sfrplus.DeltaE_00_mean_max.b_isUsed);										// 6
        addKeysAndValues(settings.sfrplus.Convergence_angle_max.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Convergence_angle_max), keys, vals, settings.sfrplus.Convergence_angle_max.b_isUsed);												// 7
        addKeysAndValues(settings.sfrplus.FOV_degrees_diagonal_min.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.FOV_degrees_diagonal_min), keys, vals, settings.sfrplus.FOV_degrees_diagonal_min.b_isUsed);										// 8
        addKeysAndValues(settings.sfrplus.High_pixel_saturation_fraction_max.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.High_pixel_saturation_fraction_max), keys, vals, settings.sfrplus.High_pixel_saturation_fraction_max.b_isUsed);		// 9
        addKeysAndValues(settings.sfrplus.Horizontal_bars_OK_min.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Horizontal_bars_OK_min), keys, vals, settings.sfrplus.Horizontal_bars_OK_min.b_isUsed);											// 10
        addKeysAndValues(settings.sfrplus.Low_pixel_saturation_fraction_max.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Low_pixel_saturation_fraction_max), keys, vals, settings.sfrplus.Low_pixel_saturation_fraction_max.b_isUsed);			//11
        addKeysAndValues(settings.sfrplus.Mirrored_chart.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Mirrored_chart), keys, vals, settings.sfrplus.Mirrored_chart.b_isUsed);																	//12
        addKeysAndValues(settings.sfrplus.MTF50P_CP_weighted_mean_min.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.MTF50P_CP_weighted_mean_min), keys, vals, settings.sfrplus.MTF50P_CP_weighted_mean_min.b_isUsed);							//13
        addKeysAndValues(settings.sfrplus.MTF50P_ratio_min.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.MTF50P_ratio_min), keys, vals, settings.sfrplus.MTF50P_ratio_min.b_isUsed);																//14
        addKeysAndValues(settings.sfrplus.passfail_ini_file_date.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.passfail_ini_file_date), keys, vals, settings.sfrplus.passfail_ini_file_date.b_isUsed);											//15
        addKeysAndValues(settings.sfrplus.Rotation_degrees_max.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Rotation_degrees_max), keys, vals, settings.sfrplus.Rotation_degrees_max.b_isUsed);													//16
        addKeysAndValues(settings.sfrplus.Secondary_readout_1_center_mean_min.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Secondary_readout_1_center_mean_min), keys, vals, settings.sfrplus.Secondary_readout_1_center_mean_min.b_isUsed);	//17
        addKeysAndValues(settings.sfrplus.Secondary_readout_1_outer_mean_min.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Secondary_readout_1_outer_mean_min), keys, vals, settings.sfrplus.Secondary_readout_1_outer_mean_min.b_isUsed);		//18
        addKeysAndValues(settings.sfrplus.Secondary_readout_1_outer_min_min.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Secondary_readout_1_outer_min_min), keys, vals, settings.sfrplus.Secondary_readout_1_outer_min_min.b_isUsed);			//19
        addKeysAndValues(settings.sfrplus.Secondary_readout_1_outer_quadrant_delta_max.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Secondary_readout_1_outer_quadrant_delta_max), keys, vals, settings.sfrplus.Secondary_readout_1_outer_quadrant_delta_max.b_isUsed);			//20
        addKeysAndValues(settings.sfrplus.Secondary_readout_1_outer_quadrant_mean_min_min.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Secondary_readout_1_outer_quadrant_mean_min_min), keys, vals, settings.sfrplus.Secondary_readout_1_outer_quadrant_mean_min_min.b_isUsed);//21
        addKeysAndValues(settings.sfrplus.Secondary_readout_2_center_mean_min.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Secondary_readout_2_center_mean_min), keys, vals, settings.sfrplus.Secondary_readout_2_center_mean_min.b_isUsed);	//22
        addKeysAndValues(settings.sfrplus.Secondary_readout_2_outer_mean_min.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Secondary_readout_2_outer_mean_min), keys, vals, settings.sfrplus.Secondary_readout_2_outer_mean_min.b_isUsed);		//23
        addKeysAndValues(settings.sfrplus.Secondary_readout_2_outer_min_min.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Secondary_readout_2_outer_min_min), keys, vals, settings.sfrplus.Secondary_readout_2_outer_min_min.b_isUsed);			//24
        addKeysAndValues(settings.sfrplus.Secondary_readout_2_outer_quadrant_delta_max.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Secondary_readout_2_outer_quadrant_delta_max), keys, vals, settings.sfrplus.Secondary_readout_2_outer_quadrant_delta_max.b_isUsed);			//25
        addKeysAndValues(settings.sfrplus.Secondary_readout_2_outer_quadrant_mean_min_min.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Secondary_readout_2_outer_quadrant_mean_min_min), keys, vals, settings.sfrplus.Secondary_readout_2_outer_quadrant_mean_min_min.b_isUsed);//26
        addKeysAndValues(settings.sfrplus.Stepchart_expected_detected.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.Stepchart_expected_detected), keys, vals, settings.sfrplus.Stepchart_expected_detected.b_isUsed);							//27
        addKeysAndValues(settings.sfrplus.upside_down.name.toStdString().c_str(), writeEntValueString(settings.sfrplus.upside_down), keys, vals, settings.sfrplus.upside_down.b_isUsed);																			//28
