...
QString AcquisitionDeviceInfo::extractDeviceName(mwArray structObj)
{
	return extractStringField(structObj, "DeviceName");
}

QString AcquisitionDeviceInfo::extractAdaptorName(mwArray structObj)
{
	return extractStringField(structObj, "Adaptor");
}

QString AcquisitionDeviceInfo::extractDefaultFormat(mwArray structObj)
{
	return extractStringField(structObj, "DefaultFormat");
}

std::vector<QString> AcquisitionDeviceInfo::extractSupportedFormats(mwArray structObj)
{
	...
	if (hasField(structObj, fieldName.c_str())) {
		...
		if (numFormats > 0 && field.ClassID() == mxCELL_CLASS) {
            ...
            for (size_t idx = 1; idx <= numFormats; ++idx) {
				formatString = field.Get(1, idx).ToString();
                formats[idx - 1] = QString(formatString);
			}
...
bool AcquisitionDeviceInfo::hasField(mwArray structObj, const char * str)
{
	...
	if (structObj.ClassID() == mxSTRUCT_CLASS) {
        size_t numFields = static_cast<size_t>(structObj.NumberOfFields());

		if (numFields > 0) {
            std::vector<mwString> fields(numFields);
            for (size_t idx = 1; idx <= numFields; ++idx) {
                fields[idx - 1] = mwString(structObj.GetFieldName(static_cast<int>(idx)));
			}
...
QString AcquisitionDeviceInfo::extractStringField(mwArray structObj, const char * fieldName)
{
	assert(structObj.ClassID() == mxSTRUCT_CLASS);
